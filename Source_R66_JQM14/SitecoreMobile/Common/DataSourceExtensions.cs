...
namespace SitecoreMobile.Common
{
    public class DataSourceExtensions
    {
        public static List<SitecoreUISearchResultItem> DataSourceSearchResults(Item contextItem, string dataSource)
        {
            ...
            List<SearchStringModel> enumerable = Sitecore.Buckets.Util.UIFilterHelpers.ParseDatasourceString(dataSource).ToList();
            ...
            {
                var query = LinqHelper.CreateQuery(searchContext, enumerable, contextItem);
                return query.ToList();
            }   
